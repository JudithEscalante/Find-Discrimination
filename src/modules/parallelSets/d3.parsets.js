...
import actions from './actions_.js'
import d3v3 from './d3v3'
//import actionsC from './actionsC.js'
...
module.exports = function(d3) {
  d3.parsets = function() {
    ...
    var  heightContainer = window.outerHeight-406
    return d3.rebind(parsets, event, "on").value(1).width(960).height(heightContainer);
...
const togglePath = (path_) => {
  const path = []
  const category =[]
  while (path_.parent) {
    if (path_.name) path.unshift(path_.name)
    if (path_.dimension) category.unshift(path_.dimension)
    path_ = path_.parent
  }
  const newWay = [path, category]
  storeUtils.dispatch(actions.togglePath(newWay))
  //storeUtils.dispatch(actionsC.toggleCategory(category))
